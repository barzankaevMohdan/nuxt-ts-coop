<template>
  ...
  >
    ...
    <label v-if="error || correct" class="text-input__label" :for="_uid">{{ error || correct }}</label>
  </div>
</template>
...
export default Vue.extend({
  ...
  computed: {
    hasBtnSlot(): boolean {
      return !!this.$slots.btn as boolean
    },
    hasTwoBtnSlot(): boolean {
      const slotsLength: number | undefined = this.$slots?.btn?.length
      return this.hasBtnSlot && slotsLength ?  slotsLength > 1 : false  as boolean
    },
  },
})
</script>
